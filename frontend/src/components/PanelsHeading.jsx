...
import { FaChevronDown, FaBolt } from 'react-icons/fa';
...
export default function PanelsHeading() {
      ...
      >
        ...
        <div className="relative bg-[#0d1f3c]/95 backdrop-blur-xl border border-electric/20 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.5),0_0_40px_rgba(0,212,255,0.1)] overflow-hidden">
          ...
          <div className="px-6 pt-5 pb-3 border-b border-white/5">
            <div>
              <h3 className="text-white font-orbitron font-bold text-base tracking-wide">
                Our <span className="text-electric">Panels</span>
              </h3>
          ...
          <div className="p-4 grid grid-cols-5 gap-3">
            {panelImages.map((item, index) => (
              <Link
                key={index}
                to={`/panels/${item.slug}`}
                onClick={() => setIsOpen(false)}
                className="group relative rounded-xl overflow-hidden border border-white/5 hover:border-electric/40 transition-all duration-300 hover:shadow-[0_0_20px_rgba(0,212,255,0.15)] aspect-[3/4] cursor-pointer block"
              >
                {/* Image */}
                <img
                  src={item.src}
